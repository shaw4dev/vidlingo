...
def test_login_and_access_me(client):
    ...
    login = client.post("/auth/login", json={"username": "bob", "password": "hunter2pw"})
...
def test_login_wrong_password_rejected(client):
    ...
    resp = client.post("/auth/login", json={"username": "carol", "password": "wrongpass1"})
...
def test_password_validation_enforced(client):
    resp = client.post("/auth/register", json={"username": "x", "password": "short"})
